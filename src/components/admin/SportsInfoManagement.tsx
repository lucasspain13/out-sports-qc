...
export const SportsInfoManagement: React.FC<SportsInfoManagementProps> = () => {
  ...
  const validateSportName = (name: string): { isValid: boolean; message?: string } => {
    // Pattern: [Season] [Year] [Sport] where Season is Spring/Summer/Fall/Winter, Year is 2025+, and Sport is capitalized
    const pattern = /^(Spring|Summer|Fall|Winter) (\d{4}) ([A-Z][a-z]+)$/;
    const match = name.match(pattern);
    
    if (!match) {
      return {
        isValid: false,
        message: "Sport name must follow the format '[Season] [Year] [Sport]' (e.g., 'Summer 2025 Kickball'). Season must be Spring/Summer/Fall/Winter, and Sport must be capitalized."
      };
    }
  ...
  const handleSave = async () => {
    try {
      // Validate sport name format
      const validation = validateSportName(formData.name);
      if (!validation.isValid) {
        setError(validation.message || "Invalid sport name format");
        return;
      }

      // Parse the name to extract season, year, and sport
      const nameMatch = formData.name.match(/^(Spring|Summer|Fall|Winter) (\d{4}) ([A-Z][a-z]+)$/);
      if (!nameMatch) {
        setError("Failed to parse sport name");
        return;
      }
      
      const [, season, yearStr, sportName] = nameMatch;
      const year = parseInt(yearStr);
      
      // Auto-generate the title 
      const autoGeneratedTitle = `${formData.name} League`;
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Summer 2025 Kickball"
                    required
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Must follow format: [Season] [Year] [Sport] (e.g., "Summer 2025 Kickball")
                    <br />
                    Season: Spring, Summer, Fall, or Winter
                    <br />
                    Year: 2025 or later
                    <br />
                    Sport: Capitalize first letter (e.g., Kickball, Basketball)
                  </p>
                </div>
