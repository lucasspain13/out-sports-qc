...
interface TeamCardProps {
  ...
  delay: number;
  teamRecord: { wins: number; losses: number } | undefined;
}

const TeamCard: React.FC<TeamCardProps> = ({ team, platform, delay, teamRecord }) => {
          ...
          <div className="space-y-4">
            {teams.map((team, index) => (
              <TeamCard
                ...
                platform={platform}
                delay={index * 0.1}
                teamRecord={teamRecords[team.id]}
              />
            ))}
          </div>
        ) : (
